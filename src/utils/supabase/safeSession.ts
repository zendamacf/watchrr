...
export class SupabaseSafeSession {
  ...
  private async parseAccessToken(accessToken: string): Promise<SupabaseSafeUserResponse> {
    try {
      const { payload } = await jwtVerify<SupabaseSafeUser>(accessToken, this.jwtSecret);
      return {
        data: {
  ...
  public async getUser(): Promise<SupabaseSafeUserResponse> {
    ...
    const { data: userData, error } = await this.parseAccessToken(token.access_token);
