# TODO


## Maybe

* Set up e2e testing (Playwright or similar; full page flows).

## Done

* Logout on 401 response from API.
* Increase test coverage.
* Add auth endpoint for logging in
* Add middleware for autenticating JWT token
* Add auth endpoint for signing up
* Switch auth to use new endpoints
* Switch signout to just clear any local cookie
* Add tests.
* Set up CI/CD for automatic database migrations after MR merges
* Add CI/CD for automatic branching of Neon database for tests.
* Manually migrate data from Supabase database to Neon
* Use device theme as default.
